...
def pdf_report_main():
    ...
    id_grabber = int(input("Enter the ID of the record you want to create a report from: "))
    print_out_comm = f"SELECT * FROM persons WHERE id = {id_grabber};"

    cursor.execute(print_out_comm)
    get_col = cursor.fetchone()

    file_name = "output/" + str(get_col[0]) + "_" + get_col[1] + "_" + get_col[2] + ".pdf"
